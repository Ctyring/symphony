...
class CHandlerSlot : public CHandlerBase {
   ...
   public:
    ...
    virtual VOID* GetThisAddr() { return reinterpret_cast<VOID*>(m_pThis); }

    virtual void EmptyThisAddr() { m_pThis = NULL; }
...
class CHandlerManager {
   public:
    ...
    ~CHandlerManager() { ClearAllHandler(); }

    template <typename TClass, typename TParam>
    bool RegisterMessageHandle(int nMsgID,
                               BOOL (TClass::*FuncPtr)(TParam*),
                               TClass* pObj) {
        ...
        std::map<int, std::vector<CHandlerBase*>*>::iterator it =
            m_mapHandler.find(nMsgID);
    ...
    bool UnregisterMessageHandle(int nMsgID, T* pObj) {
        std::map<int, std::vector<CHandlerBase*>*>::iterator itor =
            m_mapHandler.find(nMsgID);
        ...
        for (std::vector<CHandlerBase*>::iterator itor = vtHandler->begin();
             itor != vtHandler->end(); ++itor) {
    ...
    bool FireMessage(int nMsgID, TParam* pData) {
        ...
        for (std::vector<CHandlerBase*>::iterator itorHandler =
                 vtHandler->begin();
             itorHandler != vtHandler->end(); itorHandler++) {
    ...
    bool ClearAllHandler() {
        for (std::map<int, std::vector<CHandlerBase*>*>::iterator itor =
                 m_mapHandler.begin();
             itor != m_mapHandler.end(); itor++) {
            std::vector<CHandlerBase*>* pVtHandler = itor->second;
            for (std::vector<CHandlerBase*>::iterator itor2 =
                     pVtHandler->begin();
                 itor2 != pVtHandler->end(); itor2++) {
                delete *itor2;
            }
