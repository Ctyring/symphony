...
static symphony::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
    symphony::Config::Lookup("http.request.buffer_size",
                             (uint64_t)(4 * 1024),
                             "http request buffer size");

static symphony::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
    symphony::Config::Lookup("http.request.max_body_size",
                             (uint64_t)(64 * 1024 * 1024),
                             "http request max body size");
static symphony::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
    symphony::Config::Lookup("http.response.buffer_size",
                             (uint64_t)(4 * 1024),
                             "http response buffer size");

static symphony::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
    symphony::Config::Lookup("http.response.max_body_size",
                             (uint64_t)(64 * 1024 * 1024),
...
void on_request_http_field(void* data,
    ...
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if (flen == 0) {
        SYMPHONY_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
HttpRequestParser::HttpRequestParser() : m_error(0) {
    m_data.reset(new symphony::http::HttpRequest);
...
HttpResponseParser::HttpResponseParser() : m_error(0) {
    m_data.reset(new symphony::http::HttpResponse);
