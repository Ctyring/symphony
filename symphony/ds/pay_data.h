...
struct PayDataObject : public ShareObject {
    ...
    BOOL Create(symphony::MySQL::ptr pdb) {
        static CDBStoredProcedure csp(
            "INSERT INTO payment (roleid, orderid,thirdid,channel, status, money, finish_time, send_time, productid, buyid) \
			VALUES(?,?,?,?,?,?,?,?,?,?);");
    ...
    BOOL Update(symphony::MySQL::ptr pdb) {
        static CDBStoredProcedure csp(
            "REPLACE INTO payment (roleid, orderid,thirdid, channel, status, money, finish_time, send_time, productid, buyid) \
			VALUES(?,?,?,?,?,?,?,?,?,?);");
        csp.set_uint64(0, m_uRoleID);
        csp.set_string(1, m_szOrderID, strlen(m_szOrderID));
        csp.set_string(2, m_szThirdID, strlen(m_szThirdID));
    ...
    BOOL Delete(symphony::MySQL::ptr pdb) {
        static CDBStoredProcedure csp(
            "delete from payment where roleid = ? and orderid=?");
        csp.set_uint64(0, m_uRoleID);
        csp.set_string(1, m_szOrderID, strlen(m_szOrderID));
        return pDB->Execute(&csp);
    }
