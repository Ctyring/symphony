...
class AVLTree {
   ...
   private:
    ...
    bool InsertInner(TNodeTypePtr& pParentNode,
                     TNodeTypePtr pInsertNode);       // 插入
...
TreeNode<TKey, TValue>* AVLTree<TKey, TValue>::MoveNext(TNodeTypePtr pNode) {
    ...
    while (pTempNode != NULL &&
           pNode == (TreeNode<TKey, TValue>*)pTempNode->m_pRight) {
        pNode = pTempNode;
...
void AVLTree<TKey, TValue>::SingRotateLeft(TNodeTypePtr& pNode) {
    ...
    pNode->m_nHeight =
        Max(GetHeight(pNode->m_pLeft), GetHeight(pNode->m_pRight)) + 1;
    pTempNode->m_nHeight =
        Max(GetHeight(pTempNode->m_pLeft), GetHeight(pNode)) + 1;
    pNode = pTempNode;
...
void AVLTree<TKey, TValue>::SingRotateRight(TNodeTypePtr& pNode) {
    ...
    pNode->m_nHeight =
        Max(GetHeight(pNode->m_pLeft), GetHeight(pNode->m_pRight)) + 1;
    pTempNode->m_nHeight =
        Max(GetHeight(pTempNode->m_pRight), GetHeight(pNode)) + 1;
...
bool AVLTree<TKey, TValue>::InsertInner(TNodeTypePtr& pParentNode,
    ...
    if (pParentNode->m_Key > pInsertNode->m_Key) {
        ...
        if ((GetHeight(pParentNode->m_pLeft) -
             GetHeight(pParentNode->m_pRight)) >= 2) {
        ...
        if ((GetHeight(pParentNode->m_pRight) -
             GetHeight(pParentNode->m_pLeft)) >= 2) {
    ...
    pParentNode->m_nHeight =
        Max(GetHeight(pParentNode->m_pLeft), GetHeight(pParentNode->m_pRight)) +
        1;
...
template <typename TKey, typename TValue>
TreeNode<TKey, TValue>* AVLTree<TKey, TValue>::FindInner(TNodeTypePtr pNode,
                                                         TKey Key) {
...
bool AVLTree<TKey, TValue>::DeleteInner(TNodeTypePtr& pNode, TKey Key) {
    ...
    if (Key < pNode->m_Key) {
        ...
        if ((GetHeight(pNode->m_pRight) - GetHeight(pNode->m_pLeft)) >= 2) {
            if ((pNode->m_pRight->m_pLeft != NULL) &&
                (GetHeight(pNode->m_pRight->m_pLeft) >
                 GetHeight(pNode->m_pRight->m_pRight))) {
        ...
        if ((GetHeight(pNode->m_pLeft) - GetHeight(pNode->m_pRight)) >= 2) {
            if (pNode->m_pLeft->m_pRight != NULL &&
                (GetHeight(pNode->m_pLeft->m_pRight) >
                 GetHeight(pNode->m_pLeft->m_pLeft))) {
    ...
    } else  // 如果相等,此节点就是要删除的节点
    {
        if (pNode->m_pLeft && pNode->m_pRight)  // 此节点有两个儿子
        {
            ...
            if (!DeleteInner(pNode->m_pRight,
                             pTempNode->m_Key))  // 删除右子树中最小值的节点
            {
                return false;
            }
