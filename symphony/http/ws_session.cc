...
symphony::ConfigVar<uint32_t>::ptr g_websocket_message_max_size =
    symphony::Config::Lookup("websocket.message.max_size",
...
HttpRequest::ptr WSSession::handleShake() {
    ...
    do {
        req = recvRequest();
        if (!req) {
            SYMPHONY_LOG_INFO(g_logger) << "invalid http request";
            break;
        }
        if (strcasecmp(req->getHeader("Upgrade").c_str(), "websocket")) {
            SYMPHONY_LOG_INFO(g_logger) << "http header Upgrade != websocket";
            break;
        }
        if (strcasecmp(req->getHeader("Connection").c_str(), "Upgrade")) {
            SYMPHONY_LOG_INFO(g_logger) << "http header Connection != Upgrade";
            break;
        }
        ...
        if (key.empty()) {
            SYMPHONY_LOG_INFO(g_logger)
                << "http header Sec-WebSocket-Key = null";
            break;
        }

        std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
        v = symphony::base64encode(symphony::sha1sum(v));
        ...
        sendResponse(rsp);
        SYMPHONY_LOG_DEBUG(g_logger) << *req;
        SYMPHONY_LOG_DEBUG(g_logger) << *rsp;
        return req;
    } while (false);
    if (req) {
        SYMPHONY_LOG_INFO(g_logger) << *req;
    }
    return nullptr;
}
...
WSFrameMessage::ptr WSRecvMessage(Stream* stream, bool client) {
    ...
    do {
        ...
        SYMPHONY_LOG_DEBUG(g_logger) << "WSFrameHead " << ws_head.toString();

        if (ws_head.opcode == WSFrameHead::PING) {
            SYMPHONY_LOG_INFO(g_logger) << "PING";
            if (WSPong(stream) <= 0) {
                break;
            }
            ...
            if (!client && !ws_head.mask) {
                SYMPHONY_LOG_INFO(g_logger) << "WSFrameHead mask != 1";
                break;
            }
            uint64_t length = 0;
            if (ws_head.payload == 126) {  // 表示还需要再读16bit(2字节)
                uint16_t len = 0;
                if (stream->readFixSize(&len, sizeof(len)) <= 0) {
                    break;
                }
                length = symphony::byteswapOnLittleEndian(len);
            } else if (ws_head.payload == 127) {  // 表示还需要再读64bit(8字节)
                uint64_t len = 0;
                if (stream->readFixSize(&len, sizeof(len)) <= 0) {
                    break;
                }
            ...
            if (ws_head.fin) {
                SYMPHONY_LOG_DEBUG(g_logger) << data;
                return WSFrameMessage::ptr(
                    new WSFrameMessage(opcode, std::move(data)));
            }
        } else {
            SYMPHONY_LOG_DEBUG(g_logger) << "invalid opcode=" << ws_head.opcode;
        }
    } while (true);
    stream->close();
    return nullptr;
...
int32_t WSSendMessage(Stream* stream,
    ...
    do {
        ...
        if (ws_head.payload == 126) {
            uint16_t len = size;
            len = symphony::byteswapOnLittleEndian(len);
            if (stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        } else if (ws_head.payload == 127) {
            uint64_t len = symphony::byteswapOnLittleEndian(size);
            if (stream->writeFixSize(&len, sizeof(len)) <= 0) {
                break;
            }
        }
