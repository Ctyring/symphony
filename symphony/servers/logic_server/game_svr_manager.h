...
class CGameSvrMgr {
   ...
   public:
    ...
    BOOL TakeCopyRequest(UINT64 uID,
                         INT32 dwCamp,
                         UINT32 dwCopyID,
                         UINT32 dwCopyType);
   ...
   private:
    ...
    BOOL SendCreateSceneCmd(UINT32 dwServerID,
                            UINT32 dwCopyID,
                            UINT32 dwCopyType,
                            UINT64 CreateParam,
                            UINT32 dwPlayerNum);

    BOOL SendPlayerToCopy(UINT64 u64ID,
                          UINT32 dwServerID,
                          UINT32 dwCopyID,
                          UINT32 dwCopyGuid,
                          UINT32 dwCamp);

    BOOL GetMainCityInfo(UINT32 dwCopyID,
                         UINT32& dwServerID,
                         UINT32& nConnID,
                         UINT32& dwCopyGuid);
    ...
    BOOL CreateScene(UINT32 dwCopyID,
                     UINT64 CreateParam,
                     UINT32 dwPlayerNum,
                     UINT32 dwCopyType);
