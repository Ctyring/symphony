...
#include "symphony/ds/timer_manager.h"

CPlayerManager::CPlayerManager() {
    m_nMaxCacheNum =
        CConfigFile::GetInstancePtr()->GetIntValue("max_cache_num");
    if (m_nMaxCacheNum <= 10) {
        m_nMaxCacheNum = 3000;
    }
    TimerManager::GetInstancePtr()->AddFixTimer(
        0, 1, &CPlayerManager::ZeroTimer, this);
}
...
BOOL CPlayerManager::BroadMessageToAll(INT32 nMsgID,
                                       const google::protobuf::Message& pdata) {
    char szBuff[10240] = {0};
    if (pdata.ByteSize() >= 10240) {
        return FALSE;
    }

    if (!pdata.SerializePartialToArray(szBuff, pdata.ByteSize())) {
        return FALSE;
    }

    symphony::proto::BroadMessageNotify Nty;
    ...
    ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        dwProxyID, MSG_BROAD_MESSAGE_NOTIFY, 0, 0, Nty);
    ...
    for (; pNode != NULL; pNode = MoveNext(pNode)) {
        ...
        if (pTempObj->IsOnline()) {
            ...
            CRoleModule* pRoleModule =
                (CRoleModule*)pTempObj->GetModuleByType(MT_ROLE);
            if (pRoleModule == NULL) {
                return false;
            }
