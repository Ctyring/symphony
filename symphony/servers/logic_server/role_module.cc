...
#include "symphony/ds/server_define.h"
#include "symphony/ds/static_data.h"
#include "symphony/ds/static_struct.h"
...
BOOL CRoleModule::OnCreate(UINT64 u64RoleID) {
    StCarrerInfo* pInfo = CStaticData::GetInstancePtr()->GetCarrerInfo(
        m_pRoleDataObject->m_CarrerID);
    if (pInfo == NULL) {
        return FALSE;
    }

    if (m_pRoleDataObject == NULL) {
        return false;
    }
    ...
    for (int i = 0; i < ACTION_NUM; i++) {
        m_pRoleDataObject->m_Action[i] =
            CStaticData::GetInstancePtr()->GetActoinMaxValue(i + 1);
...
BOOL CRoleModule::InitBaseData(UINT64 u64RoleID,
                               std::string Name,
                               UINT32 dwCarrerID,
                               UINT64 u64AccountID,
                               INT32 nChannel) {
    ...
    CommonConvert::StrCopy(m_pRoleDataObject->m_szName, Name.c_str(),
                           ROLE_NAME_LEN);
...
BOOL CRoleModule::OnLogin() {
    ...
    if (m_pRoleDataObject->m_uLogoffTime < m_pRoleDataObject->m_uLogonTime) {
        ...
        CSimpleManager::GetInstancePtr()->SetLogoffTime(
            GetRoleID(), m_pRoleDataObject->m_uLogoffTime);
    }

    m_pRoleDataObject->m_uLogonTime = CommonFunc::GetCurrTime();
    m_pRoleDataObject->Unlock();
    CSimpleManager::GetInstancePtr()->SetLogonTime(
        GetRoleID(), m_pRoleDataObject->m_uLogonTime);
    return TRUE;
}
...
BOOL CRoleModule::OnLogout() {
    ...
    m_pRoleDataObject->m_nOnlineTime +=
        m_pRoleDataObject->m_uLogoffTime - m_pRoleDataObject->m_uLogonTime;
    m_pRoleDataObject->Unlock();
    CSimpleManager::GetInstancePtr()->SetLogoffTime(
        GetRoleID(), m_pRoleDataObject->m_uLogoffTime);
...
BOOL CRoleModule::ReadFromDBLoginData(symphony::proto::DBRoleLoginAck& Ack) {
    ...
    CommonConvert::StrCopy(m_pRoleDataObject->m_szName,
                           Ack.roledata().name().c_str(), ROLE_NAME_LEN);
    ...
    if (m_pRoleDataObject->m_CityCopyID == 0) {
        ...
        if (pInfo != NULL) {
            ...
            SYMPHONY_LOG_ERROR(g_logger)
                << "Error m_pRoleDataObject->m_CarrerID is 0";
        }
    }
...
BOOL CRoleModule::SaveToClientLoginData(symphony::proto::RoleLoginAck& Ack) {
    ...
    for (int i = 0; i < ACTION_NUM; i++) {
        symphony::proto::ActionItem* pItem = Ack.add_actionlist();
...
BOOL CRoleModule::CalcFightValue(INT32 nValue[PROPERTY_NUM],
...
BOOL CRoleModule::CostAction(UINT32 dwActionID, INT32 nActionNum) {
    if ((dwActionID <= 0) || (dwActionID >= ACTION_NUM)) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "CostAction Error: Inavlid dwActionID : " << dwActionID;
        return FALSE;
    }

    if (nActionNum <= 0) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "CostAction Error: Inavlid nActionNum : " << nActionNum;
        return FALSE;
    }
    ...
    if (m_pRoleDataObject->m_Action[dwActionID - 1] <
        CStaticData::GetInstancePtr()->GetActoinMaxValue(dwActionID)) {
        if (m_pRoleDataObject->m_Actime[dwActionID - 1] <= 0) {
            m_pRoleDataObject->m_Actime[dwActionID - 1] =
                CommonFunc::GetCurrTime();
        }
    } else {
...
BOOL CRoleModule::CheckActionEnough(UINT32 dwActionID, INT32 nActionNum) {
    if ((dwActionID <= 0) || (dwActionID > ACTION_NUM)) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "CheckActionEnough Error: Inavlid dwActionID : " << dwActionID;
        return FALSE;
    }

    if (nActionNum <= 0) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "CheckActionEnough Error: Inavlid nActionNum : " << nActionNum;
        return FALSE;
    }
...
UINT64 CRoleModule::GetAction(UINT32 dwActionID) {
    if ((dwActionID <= 0) || (dwActionID > ACTION_NUM)) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "GetAction Error: Inavlid dwActionID : " << dwActionID;
        return 0;
    }
...
UINT64 CRoleModule::AddAction(UINT32 dwActionID, INT64 nActionNum) {
    if ((dwActionID <= 0) || (dwActionID > ACTION_NUM)) {
        SYMPHONY_LOG_ERROR(g_logger)
            << "AddAction Error: Inavlid dwActionID : " << dwActionID;
        return 0;
    }
...
BOOL CRoleModule::UpdateAction(UINT32 dwActionID) {
    if (m_pRoleDataObject->m_Action[dwActionID - 1] >=
        ...
        if (m_pRoleDataObject->m_Actime[dwActionID - 1] > 0) {
            SYMPHONY_LOG_ERROR(g_logger)
                << "UpdateAction error  action is max, but starttime is not 0";
        }
        m_pRoleDataObject->m_Actime[dwActionID - 1] = 0;
        return FALSE;
    }
    ...
    if (m_pRoleDataObject->m_Actime[dwActionID - 1] <= 0) {
        // CLog::GetInstancePtr()->LogError("UpdateAction error  action not max,
        // but starttime is 0");
    }

    UINT64 dwTimeElapse =
        CommonFunc::GetCurrTime() - m_pRoleDataObject->m_Actime[dwActionID - 1];

    if (dwTimeElapse <
        CStaticData::GetInstancePtr()->GetActoinUnitTime(dwActionID)) {
        return FALSE;
    }
    ...
    if (m_pRoleDataObject->m_Action[dwActionID - 1] >=
        CStaticData::GetInstancePtr()->GetActoinMaxValue(dwActionID)) {
        m_pRoleDataObject->m_Action[dwActionID - 1] =
            CStaticData::GetInstancePtr()->GetActoinMaxValue(dwActionID);
        m_pRoleDataObject->m_Actime[dwActionID - 1] = 0;
    } else {
        m_pRoleDataObject->m_Actime[dwActionID - 1] =
...
UINT64 CRoleModule::AddExp(INT32 nExp) {
    ...
    while (m_pRoleDataObject->m_Level < MAX_ROLE_LEVEL) {
        StLevelInfo* pLevelInfo =
            CStaticData::GetInstancePtr()->GetCarrerLevelInfo(
                m_pRoleDataObject->m_CarrerID, m_pRoleDataObject->m_Level + 1);
        if (pLevelInfo == NULL) {
            return false;
        }
        ...
        if (m_pRoleDataObject->m_Exp >= pLevelInfo->dwNeedExp) {
            m_pRoleDataObject->Lock();
            m_pRoleDataObject->m_Exp =
                m_pRoleDataObject->m_Exp - pLevelInfo->dwNeedExp;
            m_pRoleDataObject->m_Level += 1;
            m_pRoleDataObject->Unlock();
        } else {
            break;
