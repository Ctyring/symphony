...
HttpConnection::~HttpConnection() {
    SYMPHONY_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if (!body.empty()) {
        ...
        SYMPHONY_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
                                     << " size=" << body.size();
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri,
    ...
    if (!turi) {
        SYMPHONY_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    ...
    if (!ptr) {
        ...
        if (!addr) {
            SYMPHONY_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock =
        ...
        if (!sock) {
            SYMPHONY_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if (!sock->connect(addr)) {
            SYMPHONY_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
