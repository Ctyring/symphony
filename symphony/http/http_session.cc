...
namespace symphony {
static symphony::Logger::ptr g_logger = SYMPHONY_LOG_NAME("debug");
namespace http {
...
HttpRequest::ptr HttpSession::recvRequest() {
    ...
    // SYMPHONY_LOG_INFO(g_logger) << "buff_size=" << buff_size << std::endl;
    // 偏移量
    int offset = 0;
    do {
        // 读取数据
        int len = read(data + offset, buff_size - offset);
        SYMPHONY_LOG_INFO(g_logger) << "len=" << len << std::endl;
        // 如果读完了就关闭
        if (len <= 0) {
            // SYMPHONY_LOG_INFO(g_logger) << "len <= 0" << std::endl;
            close();
            return nullptr;
        }
        // 得到数据总长度
        ...
        if (offset == (int)buff_size) {
            // SYMPHONY_LOG_INFO(g_logger) << "offset == buff_size" <<
            // std::endl;
            close();
            return nullptr;
        }
