...
bool CommonConvert::StringToPos(const char* pStr,
                                float& x,
                                float& y,
                                float& z) {
    if (pStr == NULL) {
...
bool CommonConvert::StringToBox(const char* pStr,
                                float& left,
                                float& top,
                                float& right,
                                float& bottom) {
...
std::string CommonConvert::FloatToString(float fValue,
                                         int32_t nPrecision,
                                         bool bRound) {
    char szValue[64] = {0};
...
bool CommonConvert::SpliteString(std::string strSrc,
                                 std::string strDelim,
    ...
    while (true) {
        subEnd = std::search(subStart, strSrc.end(), strDelim.begin(),
                             strDelim.end());
...
bool CommonConvert::SpliteStringByBlank(std::string strSrc,
                                        std::vector<std::string>& vtStr) {
    vtStr.clear();
...
bool CommonConvert::ReplaceString(std::string& str,
...
bool CommonConvert::StringToVector(const char* pStrValue,
                                   int32_t IntVector[],
                                   int32_t nSize,
                                   char cDelim) {
    if (pStrValue == NULL) {
...
#ifdef WIN32
    char sBuff[102400] = { 0 };
    WideCharToMultiByte(CP_UTF8, 0, wstrValue.c_str(), -1, sBuff, 102400, NULL,
NULL); std::string strRet = sBuff; return strRet; #else
    std::wstring_convert<std::codecvt_utf8<wchar_t>> converter;
...
std::string CommonConvert::Unicode_To_Ansi( std::wstring strValue )
{
    char sBuff[102400] = { 0 };
#ifdef WIN32
    WideCharToMultiByte(CP_ACP, 0, strValue.c_str(), -1, sBuff, 102400, NULL,
NULL); return std::string(sBuff); #else setlocale(LC_CTYPE, "chs");
...
bool CommonConvert::IsTextUTF8(const char* str, int32_t nLength) {
    ...
    for (i = 0; i < nLength; i++) {
        chr = *(str + i);
        if ((chr & 0x80) !=
            0)  // 判断是否ASCII编码,如果不是,说明有可能是UTF-8,ASCII用7位编码,但用一个字节存,最高位标记为0,o0xxxxxxx
        {
            bAllAscii = false;
        }
...
int CommonConvert::FromHex(const int& x) {
