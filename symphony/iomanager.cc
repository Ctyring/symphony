...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (fd_ctx->events & event) {
        ...
        SYMPHONY_ASSERT(!(fd_ctx->events & event));
    }
    ...
    SYMPHONY_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        SYMPHONY_ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC,
...
bool IOManager::cancelAll(int fd) {
    ...
    SYMPHONY_ASSERT(fd_ctx->events == 0);
    return true;
}
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        // 统一处理
        if (!cbs.empty()) {
            // SYMPHONY_LOG_DEBUG(g_logger) << "on timer cbs.size=" <<
            // cbs.size();
