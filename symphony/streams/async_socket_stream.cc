...
void AsyncSocketStream::Ctx::doRsp() {
    ...
    if (!symphony::Atomic::compareAndSwapBool(scheduler, scd,
                                              (Scheduler*)nullptr)) {
        return;
    }
    if (!scd || !fiber) {
...
bool AsyncSocketStream::start() {
    if (!m_iomanager) {
        m_iomanager = symphony::IOManager::GetThis();
    }

    if (!m_worker) {
        m_worker = symphony::IOManager::GetThis();
    }
...
void AsyncSocketStream::doRead() {
    ...
    SYMPHONY_LOG_DEBUG(g_logger) << "doRead out " << this;
    innerClose();
    m_waitSem.notify();
...
void AsyncSocketStream::doWrite() {
    ...
    SYMPHONY_LOG_DEBUG(g_logger) << "doWrite out " << this;
    {
        RWMutexType::WriteLock lock(m_queueMutex);
        m_queue.clear();
    }
...
bool AsyncSocketStream::innerClose() {
    SYMPHONY_ASSERT(m_iomanager == symphony::IOManager::GetThis());
...
AsyncSocketStream::ptr AsyncSocketStreamManager::get() {
    ...
    for (uint32_t i = 0; i < m_size; ++i) {
        auto idx = symphony::Atomic::addFetch(m_idx, 1);
