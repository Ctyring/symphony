...
struct GroupMailDataObject : public ShareObject {
    ...
    BOOL Create(symphony::MySQL::ptr pdb) {
        static CDBStoredProcedure csp(
            "REPLACE INTO mail_group (id, title, content,sender, mail_time, mailtype, channel,language, grouptype, itemdata) \
			VALUES(?,?,?,?,?,?,?,?,?,?);");
...
struct MailDataObject : public ShareObject {
    ...
    BOOL Create(symphony::MySQL::ptr pdb) {
        static CDBStoredProcedure csp(
            "REPLACE INTO mail (roleid, id, groupid, mailtype, mailstatus, senderid, sendername, title, content, mail_time, itemdata) \
			VALUES(?,?,?,?,?,?,?,?,?,?,?);");
