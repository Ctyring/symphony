...
BOOL CPlayerObject::OnLogin() {
    ...
    CGameSvrMgr::GetInstancePtr()->SendPlayerToMainCity(m_uRoleID,
                                                        GetCityCopyID());
...
BOOL CPlayerObject::SendMsgProtoBuf(INT32 nMsgID,
    ...
    return ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        m_nProxyConnID, nMsgID, GetRoleID(), m_nClientConnID, pdata);
}

BOOL CPlayerObject::SendMsgRawData(INT32 nMsgID,
                                   const char* pdata,
                                   UINT32 dwLen) {
    if (m_nProxyConnID == 0) {
    ...
    return ServiceBase::GetInstancePtr()->SendMsgRawData(
        m_nProxyConnID, nMsgID, GetRoleID(), m_nClientConnID, pdata, dwLen);
}

BOOL CPlayerObject::SendMsgToScene(INT32 nMsgID,
                                   const google::protobuf::Message& pdata) {
    INT32 nConnID =
        CGameSvrMgr::GetInstancePtr()->GetConnIDBySvrID(m_dwCopySvrID);

    if (nConnID == 0) {
        CLog::GetInstancePtr()->LogError(
            "Error SendMsgToScene MessageID:%d, CopySvrID:%ld", nMsgID,
            m_dwCopySvrID);
        return FALSE;
    }

    return ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        nConnID, nMsgID, m_uRoleID, m_dwCopyGuid, pdata);
}

UINT32 CPlayerObject::CheckCopyConditoin(UINT32 dwCopyID) {
    return TRUE;
    StCopyInfo* pCopyInfo =
        CStaticData::GetInstancePtr()->GetCopyInfo(dwCopyID);
    ERROR_RETURN_VALUE(m_uRoleID != 0, MRC_INVALID_COPYID);

    CRoleModule* pRoleModule = (CRoleModule*)GetModuleByType(MT_ROLE);
    ERROR_RETURN_VALUE(pRoleModule != NULL, MRC_UNKNOW_ERROR);

    if (!pRoleModule->CheckActionEnough(pCopyInfo->dwCostActID,
                                        pCopyInfo->dwCostActNum)) {
...
BOOL CPlayerObject::SendLeaveScene(UINT32 dwCopyGuid, UINT32 dwSvrID) {
    ...
    ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        ...
        MSG_LEAVE_SCENE_REQ, m_uRoleID, dwCopyGuid, LeaveReq);
    return TRUE;
}
...
BOOL CPlayerObject::SendPlayerChange(EChangeType eChangeType,
                                     UINT64 uIntValue1,
                                     UINT64 uIntValue2,
                                     std::string strValue) {
...
BOOL CPlayerObject::ToTransferData(TransferDataItem* pTransItem) {
    ...
    for (auto itor = pSkillModule->m_mapSkillData.begin();
         itor != pSkillModule->m_mapSkillData.end(); ++itor) {
    ...
    CPartnerModule* pPartnerModule =
        (CPartnerModule*)GetModuleByType(MT_PARTNER);
...
BOOL CPlayerObject::NotifyTaskEvent(UINT32 dwEventID,
                                    UINT32 dwParam1,
                                    UINT32 dwParam2) {
...
BOOL CPlayerObject::CalcFightDataInfo() {
    ...
    for (int i = 0; i < PROPERTY_NUM; i++) {
        m_Propertys[i] =
            PropertyValue[i] + PropertyValue[i] * PropertyPercent[i] / 10000;
    }
...
BOOL CPlayerObject::SetCopyStatus(UINT32 dwCopyGuid,
                                  UINT32 dwCopyID,
                                  UINT32 dwCopySvrID,
                                  BOOL bMainCity) {
    m_dwCopyID = dwCopyID;
