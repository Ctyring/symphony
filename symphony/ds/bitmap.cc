...
void Bitmap::writeTo(symphony::ByteArray::ptr ba) const {
...
bool Bitmap::readFrom(symphony::ByteArray::ptr ba) {
    try {
...
Bitmap::ptr Bitmap::compress() const {
    ...
    for (uint32_t i = 0; i < m_dataSize; ++i) {
        ...
        if ((cur_count / VALUE_SIZE) > 1) {
            SYMPHONY_ASSERT(cur_count % VALUE_SIZE == 0);
            // SYMPHONY_ASSERT(cur_count < (1ul << VALUE_SIZE));
            while (cur_count) {
                data[dst_cur_pos++] =
...
void Bitmap::set(uint32_t from, uint32_t size, bool v) {
    ...
    SYMPHONY_ASSERT(pos_from == 0);
    SYMPHONY_ASSERT(pos_to == 0);
