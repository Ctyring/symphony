...
static symphony::ConfigVar<std::string>::ptr g_module_path =
...
bool Module::handleRequest(symphony::Message::ptr req,
                           symphony::Message::ptr rsp,
                           symphony::Stream::ptr stream) {
...
bool Module::handleNotify(symphony::Message::ptr notify,
                          symphony::Stream::ptr stream) {
    SYMPHONY_LOG_DEBUG(g_logger)
...
bool Module::onConnect(symphony::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(symphony::Stream::ptr stream) {
    return true;
}
    ...
    for (auto& i : svrs) {
        ...
        for (auto& s : socks) {
            ...
            if (str.find("0.0.0.0") == 0) {
                ip_and_port =
                    symphony::GetIPv4() + ":" + std::to_string(addr->getPort());
            } else {
...
bool RockModule::handleRequest(symphony::Message::ptr req,
                               symphony::Message::ptr rsp,
                               symphony::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<symphony::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<symphony::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<symphony::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(symphony::Message::ptr notify,
                              symphony::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<symphony::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<symphony::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    symphony::FSUtil::ListAllFile(files, path, ".so");
