#ifndef __SYMPHONY_DS_TIMED_CACHE_H__
#define __SYMPHONY_DS_TIMED_CACHE_H__
...
template <class K, class V, class RWMutexType = symphony::RWMutex>
class TimedCache {
   private:
    struct Item {
   ...
   public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        auto sit =
            m_timed.insert(Item(k, v, expired + symphony::GetCurrentMS()));
    ...
    bool expired(const K& k, const uint64_t& ts) {
        ...
        uint64_t tts = ts + symphony::GetCurrentMS();
        if (it->second->ts == tts) {
            return true;
        }
        auto item = *it->second;
...
template <class K,
          class V,
          class RWMutexType = symphony::RWMutex,
