...
#include <mutex>
#include <string>
enum LogLevel { Log_HiInfo, Log_Error, Log_Warn, Log_Info, Log_None };
...
#define LOG_FUNCTION_LINE                                                 \
    CLog::GetInstancePtr()->LogError("Error : File:%s, Func: %s Line:%d", \
...
#define PARSE_FROM_PACKET(TYPE, VALUE)             \
    TYPE VALUE;                                    \
    ERROR_RETURN_TRUE(VALUE.ParsePartialFromArray( \
        pNetPacket->m_pDataBuffer->GetData(),      \
