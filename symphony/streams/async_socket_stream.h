#ifndef __SYMPHONY_STREAMS_ASYNC_SOCKET_STREAM_H__
#define __SYMPHONY_STREAMS_ASYNC_SOCKET_STREAM_H__
...
class AsyncSocketStream
   ...
   public:
    void setWorker(symphony::IOManager* v) { m_worker = v; }
    symphony::IOManager* getWorker() const { return m_worker; }

    void setIOManager(symphony::IOManager* v) { m_iomanager = v; }
    symphony::IOManager* getIOManager() const { return m_iomanager; }
   ...
   protected:
    symphony::FiberSemaphore m_sem;
    symphony::FiberSemaphore m_waitSem;
    ...
    // io读写调度
    symphony::IOManager* m_iomanager;
    // 业务处理调度(来自server分配)
    symphony::IOManager* m_worker;
