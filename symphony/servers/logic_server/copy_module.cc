...
BOOL CCopyModule::OnDestroy() {
    for (auto itor = m_mapCopyData.begin(); itor != m_mapCopyData.end();
         itor++) {
        itor->second->Release();
    }
...
BOOL CCopyModule::ReadFromDBLoginData(symphony::proto::DBRoleLoginAck& Ack) {
    const symphony::proto::DBCopyData& CopyData = Ack.copydata();
    for (int i = 0; i < CopyData.copylist_size(); i++) {
        const symphony::proto::DBCopyItem& CopyItem = CopyData.copylist(i);

        CopyDataObject* pObject =
            DataPool::CreateObject<CopyDataObject>(ESD_COPY, FALSE);
    ...
    for (int i = 0; i < CopyData.chapterlist_size(); i++) {
        const symphony::proto::DBChapterItem& ChapterItem =
            CopyData.chapterlist(i);
        ChapterDataObject* pObject =
            DataPool::CreateObject<ChapterDataObject>(ESD_CHAPTER, FALSE);
        ...
        m_mapChapterData.insert(std::make_pair(
            pObject->m_dwCopyType << 16 | pObject->m_dwChapter, pObject));
    }

    return TRUE;
}

BOOL CCopyModule::SaveToClientLoginData(symphony::proto::RoleLoginAck& Ack) {
    // 	for (auto itor = m_mapCopyData.begin(); itor != m_mapCopyData.end();
    // itor++)
    // 	{
    ...
    // 	}
    //
    // 	for (auto itor = m_mapChapterData.begin(); itor !=
    // m_mapChapterData.end(); itor++)
    // 	{
...
BOOL CCopyModule::CalcFightValue(INT32 nValue[PROPERTY_NUM],
...
BOOL CCopyModule::OnMainCopyResult(symphony::proto::BattleResultNty* pNty,
                                   INT32 nIndex) {
    const symphony::proto::ResultPlayer& pResultPlayer =
        pNty->playerlist(nIndex);

    symphony::proto::MainCopyResultNty ResultNty;
    ...
    StCopyInfo* pCopyInfo =
        CStaticData::GetInstancePtr()->GetCopyInfo(pNty->copyid());
    if (pCopyInfo == NULL) {
        return FALSE;
    }

    std::vector<StItemData> vtItemList;
    CStaticData::GetInstancePtr()->GetItemsFromAwardID(
        pCopyInfo->dwAwardID, pRoleModule->m_pRoleDataObject->m_CarrerID,
        vtItemList);
    ...
    for (std::vector<StItemData>::size_type i = 0; i < vtItemList.size(); i++) {
        ...
        symphony::proto::ItemData* pItemData = ResultNty.add_itemlist();
        pItemData->set_itemid(vtItemList[i].dwItemID);
        pItemData->set_itemnum(vtItemList[i].dwItemNum);
    }

    pRoleModule->AddExp(pCopyInfo->dwGetExpRation *
                        pRoleModule->m_pRoleDataObject->m_Level);

    pRoleModule->CostAction(pCopyInfo->dwCostActID, pCopyInfo->dwCostActNum);

    m_pOwnPlayer->SendMsgProtoBuf(symphony::proto::MSG_MAINCOPY_RESULT_NTY,
                                  ResultNty);
...
ChapterDataObject* CCopyModule::GetChapter(UINT32 dwChapter,
                                           UINT32 dwCopyType) {
