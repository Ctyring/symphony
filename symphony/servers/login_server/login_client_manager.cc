...
BOOL CLoginClientMgr::CheckClientMessage(INT32 nConnID, INT32 nMsgID) {
    ...
    if (pLoginClient == NULL) {
        if (nMsgID != symphony::proto::MSG_CHECK_VERSION_REQ) {
            return FALSE;
        }
    ...
    if (pLoginClient->m_ClientStatue == ECS_VER_CHECKED) {
        if (nMsgID == symphony::proto::MSG_ACCOUNT_LOGIN_REQ ||
            nMsgID == symphony::proto::MSG_ACCOUNT_REG_REQ) {
    ...
    if (pLoginClient->m_ClientStatue == ECS_PSD_CHECKED) {
        if (nMsgID == symphony::proto::MSG_SERVER_LIST_REQ) {
            return TRUE;
        }
