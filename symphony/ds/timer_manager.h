...
class CTimerSlotBase {
   public:
    ...
    virtual BOOL operator()(INT32 pData) { return TRUE; }
...
class EngineClass TimerManager {
   ...
   public:
    template <typename T>
    BOOL AddFixTimer(INT32 nSec,
                     INT32 nData,
                     BOOL (T::*FuncPtr)(INT32),
                     T* pObj) {
    ...
    UINT64
    m_uInitTime;  // 定时器开始工作时间(不对开始工作时间之前的定时器发生作用)
   public:
};
