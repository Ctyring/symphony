...
bool NameServerModule::handleRockRequest(symphony::RockRequest::ptr request,
                                         symphony::RockResponse::ptr response,
                                         symphony::RockStream::ptr stream) {
    symphony::Atomic::addFetch(s_request_count, 1);
...
bool NameServerModule::handleRockNotify(symphony::RockNotify::ptr notify,
                                        symphony::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(symphony::Stream::ptr stream) {
    symphony::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if (!rockstream) {
        SYMPHONY_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        SYMPHONY_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(symphony::Stream::ptr stream) {
    symphony::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if (!rockstream) {
        SYMPHONY_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if (addr) {
        SYMPHONY_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(symphony::RockStream::ptr rs) {
    symphony::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(symphony::RockRequest::ptr request,
                                      symphony::RockResponse::ptr response,
                                      symphony::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if (!rr) {
        SYMPHONY_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(symphony::RockStream::ptr rs,
                           NSClientInfo::ptr new_value) {
    if (!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    // symphony::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<symphony::RockStream::ptr> NameServerModule::getStreams(
    const std::string& domain) {
    symphony::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end()
               ? std::set<symphony::RockStream::ptr>()
               : it->second;
}
...
bool NameServerModule::handleQuery(symphony::RockRequest::ptr request,
                                   symphony::RockResponse::ptr response,
                                   symphony::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if (!qreq) {
        SYMPHONY_LOG_ERROR(g_logger) << "invalid query request from: "
                                     << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(symphony::RockStream::ptr rs,
