#ifndef __GEM_DATA_OBJECT_H__
#define __GEM_DATA_OBJECT_H__
#include "symphony/common/shared_memory.h"

struct GemDataObject : public ShareObject {
    GemDataObject() {
        m_uRoleID = 0;
        m_uGuid = 0;
        m_GemID = 0;
        m_StrengthLvl = 0;
        m_RefineLevel = 0;
        m_StarLevel = 0;
        m_RefineExp = 0;
        m_StarExp = 0;
        m_EquipPos = 0;
    }

    UINT64 m_uRoleID;     // 角色ID
    UINT64 m_uGuid;       // guid
    UINT32 m_GemID;       // 宝石ID
    INT32 m_StrengthLvl;  // 强化等级
    INT32 m_RefineLevel;  // 精炼等级
    INT32 m_StarLevel;    // 星级
    INT32 m_RefineExp;    // 精验的经验
    INT32 m_StarExp;      // 星级经验
    BOOL m_EquipPos;      // 当前使用中

    BOOL Create(symphony::MySQL::ptr pdb) {
        std::string szSql =
            "REPLACE INTO gem (guid, roleid, gemid, pos, strengthlvl, refinelvl, starlvl) \
            VALUES(?,?,?,?,?,?,?);";
        auto stmt = pdb->prepare(szSql);
        if (!stmt) {
            return FALSE;
        }
        stmt->bindUint64(0, m_uGuid);
        stmt->bindUint64(1, m_uRoleID);
        stmt->bindUint32(2, m_GemID);
        stmt->bindInt32(3, m_EquipPos);
        stmt->bindInt32(4, m_StrengthLvl);
        stmt->bindInt32(5, m_RefineLevel);
        stmt->bindInt32(6, m_StarLevel);
        return stmt->execute() == 0;
    }

    BOOL Update(symphony::MySQL::ptr pdb) {
        std::string szSql =
            "REPLACE INTO gem (guid, roleid, gemid, pos, strengthlvl, refinelvl, starlvl) \
            VALUES(?,?,?,?,?,?,?);";
        auto stmt = pdb->prepare(szSql);
        if (!stmt) {
            return FALSE;
        }
        stmt->bindUint64(0, m_uGuid);
        stmt->bindUint64(1, m_uRoleID);
        stmt->bindUint32(2, m_GemID);
        stmt->bindInt32(3, m_EquipPos);
        stmt->bindInt32(4, m_StrengthLvl);
        stmt->bindInt32(5, m_RefineLevel);
        stmt->bindInt32(6, m_StarLevel);
        return stmt->execute() == 0;
    }

    BOOL Delete(symphony::MySQL::ptr pdb) {
        std::string szSql = "delete from gem where guid = ?";
        auto stmt = pdb->prepare(szSql);
        if (!stmt) {
            return FALSE;
        }
        stmt->bindUint64(0, m_uGuid);
        return stmt->execute() == 0;
    }
};

#endif  //__GEM_DATA_OBJECT_H__