...
class SharedMemoryBase {
   public:
    SharedMemoryBase(INT32 nModuleID,
                     INT32 rawblockSize,
                     INT32 nCountPerPage,
                     BOOL noCreate = false);
...
class DataWriterBase {
   public:
    ...
    virtual BOOL SaveModifyToDB(symphony::MySQL::ptr pdb) = 0;
...
class DataWriter : public DataWriterBase {
   public:
    ...
    INT32 GetErrorCount() { return m_nErrorCount; }

    /**数据库修改*/
    BOOL SaveModifyToDB(symphony::MySQL::ptr pdb) {
        /// 共享内存不存在直接返回
        ...
        INT32 nCreateCount = 0, nUpdateCount = 0, nDeleteCount = 0,
              nRealseCount = 0;
        ...
        for (INT32 r = 0; r < nTemblockSize; r++) {
            ...
            if (pdata == NULL) {
                CLog::GetInstancePtr()->LogError(
                    "SaveModifyToDB Error pData is NULL, "
                    "ModuleID:[%d]--RawIndex:[%d]",
                    m_nModuleID, r);
                m_nErrorCount++;
                continue;
            }
            ...
            if (pdata->GetCheckCode() != BLOCK_CHECK_CODE) {
                CLog::GetInstancePtr()->LogError(
                    "SaveModifyToDB Error Memory Crash, "
                    "ModuleID:[%d]--RawIndex:[%d]",
                    m_nModuleID, r);
                m_nErrorCount++;
                continue;
            ...
            if (pdata->IsRelease()) {
                ...
                if ((lastMotifyTime > 0) &&
                    (afterTime < beforeTime || lastMotifyTime > beforeTime)) {
                    pBlock->m_beforeTime = time(0);
                    if (!pdata->Update(pdb)) {
                        m_nErrorCount++;
                        continue;
        ...
        if (nCreateCount > 0 || nCreateCount > 0 || nUpdateCount > 0 ||
            nDeleteCount > 0 || nRealseCount > 0 || m_nErrorCount > 0) {
            CLog::GetInstancePtr()->LogInfo(
                "ModuleID:[%02d]--Create:[%d]--Update:[%d]--Delete:[%d]--"
                "Release:[%d]--Error:[%d]--Time:[%d]",
                m_nModuleID, nCreateCount, nUpdateCount, nDeleteCount,
                nRealseCount, m_nErrorCount, uTickEnd - uTickStart);
        }
