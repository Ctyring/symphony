...
bool Vector2D::FromString(std::string str) {
    m_x = CommonConvert::StringToFloat(
        str.substr(0, str.find_first_of(',')).c_str());
    m_y =
        CommonConvert::StringToFloat(str.substr(str.find_last_of(',')).c_str());
    return true;
}

Vector2D Vector2D::operator-(const Vector2D& rkVector) const {
    return Vector2D(m_x - rkVector.m_x, m_y - rkVector.m_y);
}
...
bool Rect2D::Contains(CPoint2D pt) {
    return (pt.m_x >= m_fLeft && pt.m_y >= m_fTop && pt.m_x <= m_fRight &&
            pt.m_y <= m_fBottom);
}

bool Rect2D::Contains(Rect2D rcArea) {
    return (m_fLeft <= rcArea.m_fLeft && m_fBottom <= rcArea.m_fBottom &&
            rcArea.m_fRight <= m_fRight && rcArea.m_fTop <= m_fTop);
}

bool Rect2D::Intersects(Rect2D rcArea) {
    return !(m_fRight < rcArea.m_fLeft || rcArea.m_fRight < m_fLeft ||
...
Vector3D Vector3D::CrossProduct(const Vector3D& v) {
    return Vector3D(m_y * v.m_z - m_z * v.m_y, m_z * v.m_x - m_x * v.m_z,
                    m_x * v.m_y - m_y * v.m_x);
}
...
float Vector3D::AngleBetween2D(Vector3D& dest) {
    return acosf((m_x * dest.m_x + m_z * dest.m_z) /
                 sqrtf(m_x * m_x + m_z * m_z) /
