...
BOOL CMailModule::OnDestroy() {
    for (auto itor = m_mapMailData.begin(); itor != m_mapMailData.end();
         itor++) {
        itor->second->Release();
    }
...
BOOL CMailModule::ReadFromDBLoginData(symphony::proto::DBRoleLoginAck& Ack) {
    const symphony::proto::DBMailData& MailData = Ack.maildata();
    for (int i = 0; i < MailData.maillist_size(); i++) {
        const symphony::proto::DBMailItem& tMailItem = MailData.maillist(i);

        MailDataObject* pObject =
            CMailManager::GetInstancePtr()->PickUpMailData(tMailItem.guid());
        if (pObject == NULL) {
            ...
            CommonConvert::StrCopy(pObject->m_szSender,
                                   tMailItem.sender().c_str(), ROLE_NAME_LEN);
            CommonConvert::StrCopy(pObject->m_szTitle,
                                   tMailItem.title().c_str(), MAIL_TITLE_LEN);
            CommonConvert::StrCopy(pObject->m_szContent,
                                   tMailItem.content().c_str(),
                                   MAIL_CONTENT_LEN);
            memcpy((void*)&pObject->m_Items, (void*)tMailItem.items().c_str(),
                   sizeof(StMailItem) * MAIL_ITEM_COUNT);
...
BOOL CMailModule::SaveToClientLoginData(symphony::proto::RoleLoginAck& Ack) {
    return TRUE;
}

BOOL CMailModule::CalcFightValue(INT32 nValue[PROPERTY_NUM],
...
BOOL CMailModule::AddMail(symphony::proto::EMailType eMailType,
...
BOOL CMailModule::ReceiveGroupMail(GroupMailDataObject* pGroupMail) {
    ...
    CommonConvert::StrCopy(pMailObject->m_szTitle, pGroupMail->m_szTitle,
                           MAIL_TITLE_LEN);
    CommonConvert::StrCopy(pMailObject->m_szContent, pGroupMail->m_szContent,
                           MAIL_CONTENT_LEN);
    CommonConvert::StrCopy(pMailObject->m_szSender, pGroupMail->m_szSender,
                           ROLE_NAME_LEN);
...
BOOL CMailModule::NotifyChange() {
    ...
    m_pOwnPlayer->SendMsgProtoBuf(symphony::proto::MSG_MAIL_CHANGE_NTY, Nty);
