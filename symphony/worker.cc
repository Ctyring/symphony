...
static symphony::ConfigVar<
    ...
    g_worker_config = symphony::Config::Lookup(
        "workers",
        std::map<std::string, std::map<std::string, std::string> >(),
        "worker config");

WorkerGroup::WorkerGroup(uint32_t batch_size, symphony::Scheduler* s)
...
bool WorkerManager::init(
    ...
    for (auto& i : v) {
        ...
        int32_t thread_num = symphony::GetParamValue(i.second, "thread_num", 1);
        int32_t worker_num = symphony::GetParamValue(i.second, "worker_num", 1);
