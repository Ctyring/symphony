...
BOOL SharedMemoryBase::NewPage() {
    ...
    newpage.m_shm =
        CommonFunc::CreateShareMemory(m_nModuleID, m_nPageCount, nSize);
    ...
    newpage.m_pBlock =
        (_SMBlock*)(newpage.m_pdata + m_rawblockSize * m_nCountperPage);
    m_nPageCount++;
...
void SharedMemoryBase::InitToMap() {
    if (!m_bEmpty) {
        for (INT32 i = 0; i < m_nCount; ++i) {
            ...
            if (pTempBlock->m_bUse && (pTempData->GetStatus() == SMS_USE ||
                                       pTempData->GetStatus() == SMS_LOCK)) {
...
void SharedMemoryBase::ImportOtherPage() {
    while (1) {
        ...
        /// 找到头数据块的头
        page.m_pBlock =
            (_SMBlock*)(page.m_pdata + m_rawblockSize * m_nCountperPage);
 ...
 */
SharedMemoryBase::SharedMemoryBase(INT32 nModuleID,
                                   INT32 rawblockSize,
                                   INT32 nCountperPage,
                                   BOOL noCreate)
    : m_rawblockSize(rawblockSize),
      m_nCountperPage(nCountperPage),
      m_nSpace(rawblockSize + sizeof(_SMBlock)),
...

{
    ...
    if (firstpage.m_shm != NULL) {
        ...
        if (firstpage.m_pdata != NULL) {
            firstpage.m_pBlock = (_SMBlock*)(firstpage.m_pdata +
                                             m_rawblockSize * m_nCountperPage);
        ...
        if (!noCreate) {
            ...
            firstpage.m_shm =
                CommonFunc::CreateShareMemory(m_nModuleID, 0, nSize);
            if (firstpage.m_shm == NULL) {
                return;
            }

            firstpage.m_pdata =
                (CHAR*)CommonFunc::GetShareMemory(firstpage.m_shm);
...
ShareObject* SharedMemoryBase::GetObjectByRawindex(INT32 index) {
    ...
    shareMemoryPage& page = m_ShareMemoryPageMapping[whichpage];
    return reinterpret_cast<ShareObject*>(page.m_pdata +
                                          m_rawblockSize * pageindex);
}
...
SharedMemoryBase::SharedMemoryBase::mapUsedSMBlock&
SharedMemoryBase::GetUsedDataList() {
