#ifndef __SYMPHONY_DS_DICT_H__
#define __SYMPHONY_DS_DICT_H__
...
class StringDict;
template <class K, class V, class PosHash = symphony::ds::Murmur3Hash<K> >
class Dict {
   ...
   public:
    ...
    SharedArray<V> get(const K& k, bool duplicate = true) {
        ...
        symphony::RWMutex::ReadLock lock2(s_mutex[pos % MAX_MUTEX]);
        auto it =
            BinarySearch(m_datas[pos].begin(), m_datas[pos].end(), Node(k));
        // SYMPHONY_ASSERT(it == std::find(m_datas[pos].begin(),
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        symphony::RWMutex::ReadLock lock(m_mutex);
        uint32_t pos = hashvalue % m_size;
        symphony::RWMutex::WriteLock lock2(s_mutex[pos % MAX_MUTEX]);
        ...
        if (it == m_datas[pos].end()) {
            ...
            symphony::Atomic::addFetch(m_total);
        } else {
            if (it->size == (int)size) {
    ...
    void foreach (callback cb) {
        ...
        for (size_t i = 0; i < m_size; ++i) {
            symphony::RWMutex::ReadLock lock2(s_mutex[i % MAX_MUTEX]);
    ...
    bool del(const K& k) {
        ...
        if (it == m_datas[pos].end()) {
            ...
            std::sort(m_datas[pos].begin(), m_datas[pos].end());
            symphony::Atomic::subFetch(m_total);
        }
        return true;
    }
    ...
    bool writeTo(std::ostream& os, uint64_t speed = -1) {
        ...
        if (speed == (uint64_t)-1) {
            ...
            symphony::WriteFixToStreamWithSpeed(os, (const char*)&vs[0], size,
                                                speed);
        }
        ...
        if (speed == (uint64_t)-1) {
            ...
            symphony::WriteFixToStreamWithSpeed(os, (const char*)&ns[0], size,
                                                speed);
        }
   ...
   private:
    ...
    static symphony::RWMutex s_mutex[MAX_MUTEX];
};

template <class K, class V, class PosHash>
symphony::RWMutex Dict<K, V, PosHash>::s_mutex[MAX_MUTEX];
...
class StringDict {
   public:
    static uint64_t GetID(const std::string& str) {
        ...
        return symphony::murmur3_hash64(str.c_str(), str.size(), 1060627423,
                                        1050126127);
    }
    static uint64_t GetID(const char* str, const uint32_t& size) {
        ...
        return symphony::murmur3_hash64(str, size, 1060627423, 1050126127);
    }
    static uint64_t GetID(const char* str) {
        if (str == nullptr) {
            return 0;
        }
        return symphony::murmur3_hash64(str, 1060627423, 1050126127);
    }
