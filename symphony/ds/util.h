#ifndef __SYMPHONY_DS_UTIL_H__
#define __SYMPHONY_DS_UTIL_H__
...
class Murmur3Hash {
   public:
    uint32_t operator()(const T& data) {
        return symphony::murmur3_hash(&data, sizeof(T), seed);
    }
};
...
class Murmur3Hash<std::string, seed> {
   public:
    uint32_t operator()(const std::string& data) {
        return symphony::murmur3_hash(data.data(), data.size(), seed);
    }
};
...
class Murmur3Hash<std::vector<T>, seed> {
   public:
    uint32_t operator()(const std::vector<T>& data) {
        return symphony::murmur3_hash(&data[0], data.size() * sizeof(T), seed);
    }
};
...
class Murmur3Hash64 {
   public:
    uint64_t operator()(const T& data) {
        return symphony::murmur3_hash64(&data, sizeof(T), seed, seed2);
    }
};
...
class Murmur3Hash64<std::string, seed, seed2> {
   public:
    uint64_t operator()(const std::string& data) {
        return symphony::murmur3_hash64(data.data(), data.size(), seed, seed2);
    }
};
...
class Murmur3Hash64<std::vector<T>, seed, seed2> {
   public:
    uint32_t operator()(const std::vector<T>& data) {
        return symphony::murmur3_hash64(&data[0], data.size() * sizeof(T), seed,
                                        seed2);
    }
};
