...
int ZlibStream::init(Type type,
                     ...
                     Strategy strategy) {
    // 压缩等级
    SYMPHONY_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    // 选择模式
    SYMPHONY_ASSERT((window_bits >= 8 && window_bits <= 15));
    // 运行过程中的内存限制
    SYMPHONY_ASSERT((memlevel >= 1 && memlevel <= 9));
...
symphony::ByteArray::ptr ZlibStream::getByteArray() {
