...
Socket::ptr Socket::CreateTCP(symphony::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(symphony::Address::ptr address) {
...
bool Socket::bind(const Address::ptr addr) {
    ...
    if (!isValid()) {
        newSock();
        if (SYMPHONY_UNLIKELY(!isValid())) {
            return false;
        }
    }

    if (SYMPHONY_UNLIKELY(addr->getFamily() != m_family)) {
        SYMPHONY_LOG_ERROR(g_logger)
    ...
    if (uaddr) {
        ...
        if (sock->connect(uaddr)) {
            ...
            symphony::FSUtil::Unlink(uaddr->getPath(), true);
        }
    }
...
bool Socket::reconnect(uint64_t timeout_ms) {
    if (!m_remoteAddress) {
        SYMPHONY_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::listen(int backlog) {
    if (!isValid()) {
        SYMPHONY_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, symphony::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock,
                                             symphony::IOManager::WRITE);
}
...
SSLSocket::ptr SSLSocket::CreateTCP(symphony::Address::ptr address) {
