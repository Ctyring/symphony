#ifndef __SYMPHONY_DS_HASH_MULTIMAP_H__
#define __SYMPHONY_DS_HASH_MULTIMAP_H__
   ...
   public:
    ...
    bool insert(const K& k, const V& v) {
        ...
        if (it == m_datas[pos].end()) {
            ...
            symphony::Atomic::addFetch(m_elements);
        } else {
            auto iit = BinarySearch(it->val, it->val + it->size, v);
            if (iit != it->val + it->size) {
                *iit = v;
    ...
    bool insert(const K& k, const V* v, const uint32_t& size) {
        ...
        if (it == m_datas[pos].end()) {
            ...
            symphony::Atomic::addFetch(m_total);
            symphony::Atomic::addFetch(m_elements, (uint64_t)size);
        } else {
            std::set<V> news;
            ...
            it->val = datas;
            symphony::Atomic::addFetch(m_elements, news.size());
        }
        return true;
    }
    ...
    bool del(const K& k) {
        ...
        if (it == m_datas[pos].end()) {
            ...
            symphony::Atomic::subFetch(m_elements, (uint64_t)it->size);
    ...
    bool del(const K& k, const V& v) {
        ...
        if (it == m_datas[pos].end()) {
            ...
            if (it->size == 1) {
                ...
                symphony::Atomic::subFetch(m_total);
                symphony::Atomic::subFetch(m_elements);
                return true;
            }
...
symphony::RWMutex HashMultimap<K, V, PosHash>::s_mutex[MAX_MUTEX];
