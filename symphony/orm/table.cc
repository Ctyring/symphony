...
std::string Table::getFilename() const {
    return symphony::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if (!node.Attribute("name")) {
        SYMPHONY_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if (!node.Attribute("namespace")) {
        SYMPHONY_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + symphony::replace(m_namespace, ".", "/");
    symphony::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}
...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"symphony/db/db.h", "symphony/util.h"};
    for (auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = symphony::split(m_namespace, '.');
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for (auto it = m_cols.begin(); it != m_cols.end(); ++it) {
        ...
        if ((*it)->getDType() == Column::TYPE_UINT64 ||
            ...
            ss << "symphony::Time2Str(" << GetAsMemberName((*it)->getName())
               << ")"
               << ";" << std::endl;
        } else {
    ...
    ss << "    return symphony::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"symphony/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static symphony::Logger::ptr g_logger = SYMPHONY_LOG_NAME(\"orm\");"
        << std::endl;
...
#define CHECK_STMT(v)                                                   \
    ...
    ofs << "        SYMPHONY_LOG_ERROR(g_logger) << \"stmt=\" << sql"   \
        << std::endl                                                    \
        << "                 << \" errno=\""                            \
