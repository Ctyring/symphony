...
class CPlayerObject : public CHandlerManager {
   public:
    ...
    BOOL ToTransferData(symphony::proto::TransferDataItem* pTransItem);
   ...
   public:
    // 全部是操作方法
    BOOL SendIntoSceneNotify(UINT32 dwCopyGuid,
                             UINT32 dwCopyID,
                             UINT32 dwSvrID);
    BOOL SendLeaveScene(UINT32 dwCopyGuid, UINT32 dwSvrID);
    BOOL SendRoleLoginAck();
    BOOL SendPlayerChange(EChangeType eChangeType,
                          UINT64 uIntValue1,
                          UINT64 uIntValue2,
                          std::string strValue);
    ...
    BOOL SetCopyStatus(UINT32 dwCopyGuid,
                       UINT32 dwCopyID,
                       UINT32 dwCopySvrID,
                       BOOL bMainCity);
