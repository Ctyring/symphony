...
BOOL CGameSvrMgr::TakeCopyRequest(UINT64 uID,
                                  INT32 dwCamp,
                                  UINT32 dwCopyID,
                                  UINT32 dwCopyType) {
    ...
    ERROR_RETURN_TRUE(CGameSvrMgr::GetInstancePtr()->CreateScene(
        dwCopyID, uID, 1, dwCopyType));
...
BOOL CGameSvrMgr::TakeCopyRequest(UINT64 uID[],
    ...
    ERROR_RETURN_TRUE(CGameSvrMgr::GetInstancePtr()->CreateScene(
        dwCopyID, uID[0], nNum, dwCopyType));
...
BOOL CGameSvrMgr::CreateScene(UINT32 dwCopyID,
                              UINT64 uCreateParam,
                              UINT32 dwPlayerNum,
                              UINT32 dwCopyType) {
    ...
    // 向副本服务器发送创建副本的消息
    if (!SendCreateSceneCmd(dwServerID, dwCopyID, dwCopyType, uCreateParam,
                            dwPlayerNum)) {
        // 发送创建副本的消息失败
...
BOOL CGameSvrMgr::BroadMsgToAll(INT32 nMsgID, CHAR* pData, INT32 nSize) {
    for (std::map<UINT32, GameSvrInfo>::iterator itor = m_mapGameSvr.begin();
         itor != m_mapGameSvr.end(); itor++) {
        GameSvrInfo& Info = itor->second;

        ERROR_CONTINUE_EX(ServiceBase::GetInstancePtr()->SendMsgRawData(
            Info.m_nConnID, nMsgID, 0, 0, pData, nSize));
    }
...
VOID CGameSvrMgr::RegisterMessageHanler() {
    CMsgHandlerManager::GetInstancePtr()->RegisterMessageHandle(
        MSG_GAME_REGTO_LOGIC_REQ, &CGameSvrMgr::OnMsgGameSvrRegister, this);
    CMsgHandlerManager::GetInstancePtr()->RegisterMessageHandle(
        MSG_CREATE_SCENE_ACK, &CGameSvrMgr::OnMsgCreateSceneAck, this);
    CMsgHandlerManager::GetInstancePtr()->RegisterMessageHandle(
        MSG_TRANSFER_DATA_ACK, &CGameSvrMgr::OnMsgTransRoleDataAck, this);
    CMsgHandlerManager::GetInstancePtr()->RegisterMessageHandle(
        MSG_COPYINFO_REPORT_REQ, &CGameSvrMgr::OnMsgCopyReportReq, this);
    CMsgHandlerManager::GetInstancePtr()->RegisterMessageHandle(
        MSG_BATTLE_RESULT_NTY, &CGameSvrMgr::OnMsgBattleResultNty, this);
}

BOOL CGameSvrMgr::SendCreateSceneCmd(UINT32 dwServerID,
                                     UINT32 dwCopyID,
                                     UINT32 dwCopyType,
                                     UINT64 CreateParam,
                                     UINT32 dwPlayerNum) {
    ...
    return ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        GetConnIDBySvrID(dwServerID), MSG_CREATE_SCENE_REQ, 0, 0, Req);
}

UINT32 CGameSvrMgr::GetConnIDBySvrID(UINT32 dwServerID) {
    std::map<UINT32, GameSvrInfo>::iterator itor =
        m_mapGameSvr.find(dwServerID);
...
BOOL CGameSvrMgr::SendPlayerToMainCity(UINT64 u64ID, UINT32 dwCopyID) {
    ...
    CGameSvrMgr::GetInstancePtr()->GetMainCityInfo(dwCopyID, dwSvrID, nConnID,
                                                   dwCopyGuid);
...
BOOL CGameSvrMgr::SendPlayerToCopy(UINT64 u64ID,
                                   UINT32 dwServerID,
                                   UINT32 dwCopyID,
                                   UINT32 dwCopyGuid,
                                   UINT32 dwCamp) {
    ...
    ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        nConnID, MSG_TRANSFER_DATA_REQ, u64ID, dwCopyGuid, Req);
...
BOOL CGameSvrMgr::GetMainCityInfo(UINT32 dwCopyID,
                                  UINT32& dwServerID,
                                  UINT32& nConnID,
                                  UINT32& dwCopyGuid) {
...
BOOL CGameSvrMgr::AddWaitItem(UINT64 uKey,
                              UINT64 uID[],
                              INT32 dwCamp[],
                              INT32 nNum) {
...
BOOL CGameSvrMgr::OnMsgGameSvrRegister(NetPacket* pNetPacket) {
    SvrRegToSvrReq Req;
    Req.ParsePartialFromArray(pNetPacket->m_pDataBuffer->GetData(),
                              pNetPacket->m_pDataBuffer->GetBodyLenth());
    PacketHeader* pHeader =
        (PacketHeader*)pNetPacket->m_pDataBuffer->GetBuffer();

    std::map<UINT32, GameSvrInfo>::iterator itor =
        m_mapGameSvr.find(Req.serverid());
    if (itor != m_mapGameSvr.end()) {
        itor->second.m_nConnID = pNetPacket->m_nConnID;
        itor->second.m_nSvrID = Req.serverid();
    }

    m_mapGameSvr.insert(std::make_pair(
        Req.serverid(), GameSvrInfo(Req.serverid(), pNetPacket->m_nConnID)));
...
BOOL CGameSvrMgr::OnMsgCopyReportReq(NetPacket* pNetPacket) {
    ...
    for (int i = 0; i < Req.copylist_size(); i++) {
        ...
        m_mapCity.insert(std::make_pair(
            item.copyid(), CityInfo(item.copyid(), item.serverid(),
                                    pNetPacket->m_nConnID, item.copyguid())));
...
BOOL CGameSvrMgr::OnMsgCreateSceneAck(NetPacket* pNetPacket) {
    ...
    Ack.ParsePartialFromArray(pNetPacket->m_pDataBuffer->GetData(),
                              pNetPacket->m_pDataBuffer->GetBodyLenth());
    ...
    INT32 nConnID =
        CGameSvrMgr::GetInstancePtr()->GetConnIDBySvrID(Ack.serverid());
    ERROR_RETURN_FALSE(nConnID != 0);
    ...
    for (int i = 0; i < 10; i++) {
        ...
        CPlayerObject* pPlayer =
            CPlayerManager::GetInstancePtr()->GetPlayer(pWaitItem->uID[i]);
        ERROR_RETURN_FALSE(pPlayer != NULL);
        ...
        if (pPlayer->m_bMainCity) {
            pPlayer->SendLeaveScene(pPlayer->m_dwCopyGuid,
                                    pPlayer->m_dwCopySvrID);
        }

        pPlayer->SetCopyStatus(Ack.copyguid(), Ack.copyid(), Ack.serverid(),
                               FALSE);
    }

    ServiceBase::GetInstancePtr()->SendMsgProtoBuf(
        nConnID, MSG_TRANSFER_DATA_REQ, Ack.createparam(), Ack.copyguid(), Req);
...
BOOL CGameSvrMgr::OnMsgTransRoleDataAck(NetPacket* pNetPacket) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ERROR_RETURN_TRUE(Ack.copyguid() != 0);
        ERROR_RETURN_TRUE(Ack.serverid() != 0);
        pPlayer->SendIntoSceneNotify(Ack.copyguid(), Ack.copyid(),
                                     Ack.serverid());
    }
...
BOOL CGameSvrMgr::OnMsgBattleResultNty(NetPacket* pNetPacket) {
    BattleResultNty Nty;
    Nty.ParsePartialFromArray(pNetPacket->m_pDataBuffer->GetData(),
                              pNetPacket->m_pDataBuffer->GetBodyLenth());
...
BOOL CGameSvrMgr::OnMainCopyResult(BattleResultNty& Nty) {
    ...
    for (int i = 0; i < Nty.playerlist_size(); i++) {
        ...
        CPlayerObject* pPlayer =
            CPlayerManager::GetInstancePtr()->GetPlayer(Result.objectid());
        ERROR_RETURN_TRUE(pPlayer != NULL);

        CCopyModule* pCopyModule =
            (CCopyModule*)pPlayer->GetModuleByType(MT_COPY);
