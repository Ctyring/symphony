...
#include "symphony/servers/account_server/log_struct.h"
...
VOID CGameLogManager::SetLogBase(CPlayerObject* pPlayer,
                                 Log_BaseData* pLogBase) {
    pLogBase->m_uRoleID = pPlayer->GetRoleID();
    pLogBase->m_uAccountID = pPlayer->GetAccountID();
    pLogBase->m_nAreaID = CConfigFile::GetInstancePtr()->GetIntValue("areaid");
    ...
    CommonConvert::StrCopy(pLogBase->m_szRoleName, pPlayer->GetName(),
                           sizeof(pLogBase->m_szRoleName));
}
...
BOOL CGameLogManager::LogRoleLogout(CPlayerObject* pPlayer) {
    ...
    if (CommonFunc::DiffDays(pRoleModule->GetLastLogonTime(),
                             pRoleModule->GetLastLogoffTime()) > 0) {
        LogRoleLogin(pPlayer);
    }

    return TRUE;
}
BOOL CGameLogManager::LogRoleChat(CPlayerObject* pPlayer,
                                  INT32 nChatChl,
    ...
    CommonConvert::StrCopy(log.m_szTargetName, strTargetName.c_str(),
                           sizeof(log.m_szTargetName));
    CommonConvert::StrCopy(log.m_szRoleName, pPlayer->GetName(),
                           sizeof(log.m_szRoleName));
