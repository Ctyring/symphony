...
static symphony::ConfigVar<
    ...
    g_sqlite3_dbs = symphony::Config::Lookup(
        "sqlite3.dbs",
...
int SQLite3Stmt::bindTime(int idx, const time_t& value) {
    return bind(idx, symphony::Time2Str(value));
}
...
time_t SQLite3Data::getTime(int idx) {
    ...
    return symphony::Str2Time(str.c_str());
}
...
SQLite3::ptr SQLite3Manager::get(const std::string& name) {
    ...
    lock.unlock();
    std::string path = symphony::GetParamValue<std::string>(args, "path");
    if (path.empty()) {
        SYMPHONY_LOG_ERROR(g_logger)
    ...
    if (path.find(":") == std::string::npos) {
        path = symphony::EnvMgr::GetInstance()->getAbsoluteWorkPath(path);
    }
    ...
    std::string sql = symphony::GetParamValue<std::string>(args, "sql");
    if (!sql.empty()) {
        if (rt->execute(sql)) {
            SYMPHONY_LOG_ERROR(g_logger)
...
int SQLite3Manager::execute(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        SYMPHONY_LOG_ERROR(g_logger) << "SQLite3Manager::execute, get(" << name
                                     << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr SQLite3Manager::query(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        SYMPHONY_LOG_ERROR(g_logger) << "SQLite3Manager::query, get(" << name
                                     << ") fail, format=" << format;
        return nullptr;
    }
