...
SmtpClient::SmtpClient(Socket::ptr sock) : symphony::SocketStream(sock) {}

SmtpClient::ptr SmtpClient::Create(const std::string& host,
                                   uint32_t port,
                                   bool ssl) {
    symphony::IPAddress::ptr addr = symphony::Address::LookupAnyIPAddress(host);
    if (!addr) {
    ...
    Socket::ptr sock;
    if (ssl) {
        sock = symphony::SSLSocket::CreateTCP(addr);
    } else {
        sock = symphony::Socket::CreateTCP(addr);
    }
    if (!sock->connect(addr)) {
        SYMPHONY_LOG_ERROR(g_logger) << "connect smtp server: " << host << ":"
                                     << port << " ssl=" << ssl << " fail";
        return nullptr;
    }
    std::string buf;
    ...
    buf.resize(len);
    if (symphony::TypeUtil::Atoi(buf) != 220) {
        return nullptr;
    }
    rt->m_host = host;
    return rt;
...
SmtpResult::ptr SmtpClient::doCmd(const std::string& cmd, bool debug) {
    ...
    int code = symphony::TypeUtil::Atoi(buf);
    if (code >= 400) {
        return std::make_shared<SmtpResult>(
            code, symphony::replace(buf.substr(buf.find(' ') + 1), "\r\n", ""));
    }
    return nullptr;
}
...
#define DO_CMD()                \
    ...
    if (!m_authed && !email->getFromEMailAddress().empty()) {
        ...
        cmd = symphony::base64encode(
                  email->getFromEMailAddress().substr(0, pos)) +
              "\r\n";

        DO_CMD();
        // 发送密码信息的base64编码 服务端会返回 235 Authentication successful
        cmd = symphony::base64encode(email->getFromEMailPasswd()) + "\r\n";
        DO_CMD();
...
#define XX(fun)                                 \
    ...
    if (!entitys.empty()) {
        boundary = symphony::random_string(16);
