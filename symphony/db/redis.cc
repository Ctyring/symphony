...
Redis::Redis(const std::map<std::string, std::string>& conf) {
    ...
    m_port = symphony::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = symphony::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if (tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = symphony::TypeUtil::Atoi(tmp);
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if (c) {
        ...
        if (!m_passwd.empty()) {
            ...
            if (!r) {
                SYMPHONY_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":"
                                             << m_port << ", " << m_name << ")";
                return false;
            }
...
ReplyPtr Redis::getReply() {
    ...
    if (m_logEnable) {
        SYMPHONY_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host
                                     << ":" << m_port << ")(" << m_name << ")";
    }
    return nullptr;
}
...
FoxRedis::FoxRedis(symphony::FoxThread* thr,
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if (!r) {
        SYMPHONY_LOG_ERROR(g_logger) << "auth error:(" << fr->m_host << ":"
                                     << fr->m_port << ", " << fr->m_name << ")";
        return;
    }
    if (r->type != REDIS_REPLY_STATUS) {
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if (!status) {
        SYMPHONY_LOG_INFO(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host
        ...
        if (!ar->m_passwd.empty()) {
            ...
            if (rt) {
                SYMPHONY_LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
            }
        }
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    SYMPHONY_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb " << c->c.tcp.host
...
void FoxRedis::CmdCb(redisAsyncContext* ac, void* r, void* privdata) {
    ...
    if (ctx->timeout) {
        delete ctx;
        // if(ctx && ctx->fiber) {
        //     SYMPHONY_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "'
        //     timeout("
...
//     SYMPHONY_LOG_INFO(g_logger) << "DelayTimeCb";
...
bool FoxRedis::init() {
    if (m_thread == symphony::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit() {
    // SYMPHONY_LOG_INFO(g_logger) << "pinit m_status=" << m_status;
    ...
    m_context.reset(ctx, symphony::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char* fmt, va_list ap) {
    ...
    if (len == -1) {
        SYMPHONY_LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
        return nullptr;
    }
    ...
    free(buf);
    fctx.scheduler = symphony::Scheduler::GetThis();
    fctx.fiber = symphony::Fiber::GetThis();
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
    ...
    do {
        ...
        int len = redisFormatCommandArgv(&buf, argv.size(), &(args[0]),
                                         &(args_len[0]));
        if (len == -1 || !buf) {
            SYMPHONY_LOG_ERROR(g_logger) << "redis fmt error";
            return nullptr;
        }
        fctx.cmd.append(buf, len);
        free(buf);
...
void FoxRedis::pcmd(FCtx* fctx) {
    if (m_status == UNCONNECTED) {
        SYMPHONY_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port
...
FoxRedis::Ctx::Ctx(FoxRedis* r)
    : ev(nullptr),
      ...
      ,
      thread(nullptr) {
    symphony::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx() {
    // cancelEvent();
    SYMPHONY_ASSERT(thread == symphony::FoxThread::GetThis());
    // SYMPHONY_ASSERT(destory == 0);
    symphony::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    // cancelEvent();
    if (ev) {
        evtimer_del(ev);
...
void FoxRedis::Ctx::cancelEvent() {
    // if(ev) {
    //     if(thread == symphony::FoxThread::GetThis()) {
    //         evtimer_del(ev);
    //         event_free(ev);
    //     } else {
    //         auto e = ev;
...
FoxRedisCluster::FoxRedisCluster(symphony::FoxThread* thr,
                                 const std::map<std::string, std::string>& conf)
    : m_thread(thr), m_status(UNCONNECTED), m_event(nullptr) {
    m_ctxCount = 0;
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c,
    ...
    if (!r->str) {
        SYMPHONY_LOG_ERROR(g_logger) << "auth reply str error: NULL("
                                     << fr->m_host << ", " << fr->m_name << ")";
        return;
    }
    if (strcmp(r->str, "OK") == 0) {
        SYMPHONY_LOG_INFO(g_logger) << "auth ok: " << r->str << "("
                                    << fr->m_host << ", " << fr->m_name << ")";
    } else {
        SYMPHONY_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                                     << fr->m_host << ", " << fr->m_name << ")";
    }
}
...
bool FoxRedisCluster::pinit() {
    ...
    SYMPHONY_LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
    ...
    m_status = CONNECTED;
    // m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, symphony::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::Ctx::cancelEvent() {
    // SYMPHONY_LOG_INFO(g_logger) << "cancelEvent " <<
    // symphony::FoxThread::GetThis()
    //            << " - " << thread
    ...
    //    //SYMPHONY_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << "
    //    "
    //    << thread->getId()
    //    //           << " this=" << symphony::FoxThread::GetThis();
    //    //SYMPHONY_ASSERT(thread == symphony::FoxThread::GetThis());
    //}
    // SYMPHONY_ASSERT(!symphony::IOManager::GetThis());
    ////if(symphony::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////SYMPHONY_ASSERT(!symphony::Fiber::GetThis());
    ////symphony::RWMutex::WriteLock lock(mutex);
    // if(++cancel_count > 1) {
    //     return;
    // }
    // if(ev) {
...
IRedis::ptr RedisManager::get(const std::string& name) {
    ...
    if (r->getType() == IRedis::FOX_REDIS ||
        ...
        return std::shared_ptr<IRedis>(r, symphony::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init() {
    ...
    for (auto& i : m_config) {
        ...
        auto pool = symphony::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for (int n = 0; n < pool; ++n) {
            if (type == "redis") {
                symphony::Redis* rds(new symphony::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                symphony::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                symphony::Atomic::addFetch(done, 1);
            } else if (type == "redis_cluster") {
                symphony::RedisCluster* rds(
                    new symphony::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                symphony::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                symphony::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        symphony::FoxRedis* rds(new symphony::FoxRedis(
                            symphony::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
                ...
                symphony::FoxThreadMgr::GetInstance()->dispatch(
                    "redis", [this, conf, name, &done]() {
                        symphony::FoxRedisCluster* rds(
                            new symphony::FoxRedisCluster(
                                symphony::FoxThread::GetThis(), conf));
                        rds->init();
                        rds->setName(name);
