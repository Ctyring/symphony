...
ServiceItemInfo::ptr ServiceItemInfo::Create(const std::string& ip_and_port,
    ...
    auto port = symphony::TypeUtil::Atoi(ip_and_port.substr(pos + 1));
...
void ZKServiceDiscovery::start() {
    ...
    m_client.reset(new symphony::ZKClient);
    bool b =
    ...
    if (!b) {
        SYMPHONY_LOG_ERROR(g_logger) << "ZKClient init fail, hosts=" << m_hosts;
    }
...
void ZKServiceDiscovery::onZKConnect(const std::string& path,
    ...
    if (!ok) {
        SYMPHONY_LOG_ERROR(g_logger) << "onZKConnect register fail";
    }
    ...
    if (!ok) {
        SYMPHONY_LOG_ERROR(g_logger) << "onZKConnect query fail";
    }
    ...
    if (!ok) {
        SYMPHONY_LOG_ERROR(g_logger) << "onZKConnect queryData fail";
    }
}
    ...
    if (v == ZOK) {
        ...
        if (pos == std::string::npos) {
            SYMPHONY_LOG_ERROR(g_logger)
                << "existsOrCreate invalid path=" << path;
            return false;
        }
...
bool ZKServiceDiscovery::registerInfo(const std::string& domain,
    ...
    if (!v) {
        SYMPHONY_LOG_ERROR(g_logger) << "create path=" << path << " fail";
        return false;
    }
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain,
    ...
    if (service != "all") {
        ...
        if (m_selfInfo.empty()) {
            SYMPHONY_LOG_ERROR(g_logger) << "queryInfo selfInfo is null";
            return false;
        }
...
bool ZKServiceDiscovery::getChildren(const std::string& path) {
    ...
    }
    {
        ...
        if (it == m_queryInfos.end()) {
            SYMPHONY_LOG_ERROR(g_logger)
                << "get_children path=" << path << " domian=" << domain
                << " not exists";
            return false;
        }
        if (it->second.count(service) == 0 && it->second.count("all") == 0) {
            SYMPHONY_LOG_ERROR(g_logger)
                ...
                << " not exists "
                << symphony::Join(it->second.begin(), it->second.end(), ",");
            return false;
        }
    }
...
bool ZKServiceDiscovery::queryData(const std::string& domain,
                                   const std::string& service) {
    // SYMPHONY_LOG_INFO(g_logger) << "query_data domain=" << domain
    //                          << " service=" << service;
    if (service != "all") {
        std::string path = GetProvidersPath(domain, service);
...
void ZKServiceDiscovery::onZKChild(const std::string& path,
                                   ZKClient::ptr client) {
    // SYMPHONY_LOG_INFO(g_logger) << "onZKChild path=" << path;
    getChildren(path);
}

void ZKServiceDiscovery::onZKChanged(const std::string& path,
                                     ZKClient::ptr client) {
    SYMPHONY_LOG_INFO(g_logger) << "onZKChanged path=" << path;
}

void ZKServiceDiscovery::onZKDeleted(const std::string& path,
                                     ZKClient::ptr client) {
    SYMPHONY_LOG_INFO(g_logger) << "onZKDeleted path=" << path;
}

void ZKServiceDiscovery::onZKExpiredSession(const std::string& path,
                                            ZKClient::ptr client) {
    SYMPHONY_LOG_INFO(g_logger) << "onZKExpiredSession path=" << path;
    client->reconnect();
}
