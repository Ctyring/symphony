...
static symphony::ConfigVar<uint32_t>::ptr g_daemon_restart_interval =
    symphony::Config::Lookup("daemon.restart_interval",
                             (uint32_t)5,
...
std::string ProcessInfo::toString() const {
    ...
    ss << "[ProcessInfo parent_id=" << parent_id << " main_id=" << main_id
       << " parent_start_time=" << symphony::Time2Str(parent_start_time)
       << " main_start_time=" << symphony::Time2Str(main_start_time)
...
static int real_daemon(int argc,
    ...
    while (true) {
        ...
        if (pid == 0) {
            ...
            ProcessInfoMgr::GetInstance()->main_id = getpid();
            ProcessInfoMgr::GetInstance()->main_start_time = time(0);
            SYMPHONY_LOG_INFO(g_logger) << "process start pid=" << getpid();
            ...
            if (status) {
                if (status == 9) {
                    SYMPHONY_LOG_INFO(g_logger) << "killed";
                    break;
                } else {
                    SYMPHONY_LOG_ERROR(g_logger)
                ...
                SYMPHONY_LOG_INFO(g_logger) << "child finished pid=" << pid;
                break;
            }
