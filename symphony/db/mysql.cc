...
static symphony::ConfigVar<
    ...
    g_mysql_dbs = symphony::Config::Lookup(
        "mysql.dbs",
...
static MYSQL* mysql_init(std::map<std::string, std::string>& params,
    ...
    if (mysql == nullptr) {
        SYMPHONY_LOG_ERROR(g_logger) << "mysql_init error";
        return nullptr;
    }
    ...
    int port = symphony::GetParamValue(params, "port", 0);
    std::string host = symphony::GetParamValue<std::string>(params, "host");
    std::string user = symphony::GetParamValue<std::string>(params, "user");
    std::string passwd = symphony::GetParamValue<std::string>(params, "passwd");
    std::string dbname = symphony::GetParamValue<std::string>(params, "dbname");
...
bool MySQL::connect() {
    ...
    m_hasError = false;
    m_poolSize = symphony::GetParamValue(m_params, "pool", 5);
...
int MySQL::execute(const char* format, va_list ap) {
    m_cmd = symphony::StringUtil::Formatv(format, ap);
...
std::shared_ptr<MySQL> MySQL::getMySQL() {
    return MySQL::ptr(this, symphony::nop<MySQL>);
}
...
static MYSQL_RES* my_mysql_query(MYSQL* mysql, const char* sql) {
    if (mysql == nullptr) {
        SYMPHONY_LOG_ERROR(g_logger) << "mysql_query mysql is null";
        return nullptr;
    }

    if (sql == nullptr) {
        SYMPHONY_LOG_ERROR(g_logger) << "mysql_query sql is null";
        return nullptr;
    }
...
int MySQLStmt::bindTime(int idx, const time_t& value) {
    ...
    // return 0;
    return bindString(idx, symphony::Time2Str(value));
}
...
int64_t MySQLRes::getInt64(int idx) {
    return symphony::TypeUtil::Atoi(m_cur[idx]);
}
...
double MySQLRes::getDouble(int idx) {
    return symphony::TypeUtil::Atof(m_cur[idx]);
}
...
time_t MySQLRes::getTime(int idx) {
    ...
    return symphony::Str2Time(m_cur[idx]);
}
...
int MySQLTransaction::execute(const std::string& sql) {
    if (m_isFinished) {
        SYMPHONY_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
        return -1;
    }
    ...
    if (it != m_conns.end()) {
        if (!it->second.empty()) {
            ...
            if (rt->ping()) {
                ...
                SYMPHONY_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                return nullptr;
            }
        }
    }
...
int MySQLManager::execute(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        SYMPHONY_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                                     << ") fail, format=" << format;
        return -1;
    }
...
ISQLData::ptr MySQLManager::query(const std::string& name,
    ...
    auto conn = get(name);
    if (!conn) {
        SYMPHONY_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
                                     << ") fail, format=" << format;
        return nullptr;
    }
