...
void Logger::setFormatter(const std::string& val) {
    ...
    symphony::LogFormatter::ptr new_val(new symphony::LogFormatter(val));
    if (new_val->isError()) {
...
symphony::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    symphony::Config::Lookup("logs", std::set<LogDefine>(), "logs config");

struct LogIniter {
    LogIniter() {
        g_log_defines->addListener([](const std::set<LogDefine>& old_value,
                                      const std::set<LogDefine>& new_value) {
            SYMPHONY_LOG_INFO(SYMPHONY_LOG_ROOT()) << "on_logger_conf_changed";
            for (auto& i : new_value) {
                auto it = old_value.find(i);
                symphony::Logger::ptr logger;
                if (it == old_value.end()) {
                    // 新增logger
                    logger = SYMPHONY_LOG_NAME(i.name);
                } else {
                    if (!(i == *it)) {
                        // 修改的logger
                        logger = SYMPHONY_LOG_NAME(i.name);
            ...
            for (auto& i : old_value) {
                ...
                if (it == new_value.end()) {
                    // 删除logger
                    auto logger = SYMPHONY_LOG_NAME(i.name);
